...
module.exports = async member => {
  ...
  var tagKD = await `${
    member.guild.channels.get(db.fetch(`tagKanal_${member.guild.id}`))
      ? "var"
      : "yok"
  }`;
    ...
    if (db.has(`tagKanal_${member.guild.id}`) === true) {
      if (tagKD === "var") {
        member.guild.channels
          .get(tagK)
          .send(
            `**${member.user.tag}** adlı kullanıcıya \`${db.fetch(
              `tagB_${member.guild.id}`
            )}\` olarak ayarlanmış olan tag verilerek kullanıcının ismi sunucu için \`${member.nickname ||
              `${db.fetch(`tagB_${member.guild.id}`)} ${
    ...
    if (db.has(`sKanal_${member.guild.id}`) === true) {
      ...
      if (db.has(`üyelikk_${member.id}`)) {
        const embed = new Discord.RichEmbed()
          .setColor("RANDOM")
          .setDescription(
            `<a:sekilligalp:727045194033266730> Gold üye belirdi. \`${
              member.user.tag
            }\`, Ayağa kalkın \`${db.fetch(
              `sayac_${member.guild.id}`
            )}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) -
              member.guild.members
                .size}\` Kişi Kaldı <a:sekilligalp:727045194033266730>`
          );
