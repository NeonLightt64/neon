...
exports.run = (client, message, args) => {
  ...
  if (!command) {
    return message.reply(
      "Botta `" + args.slice(0).join(" ") + "` isminde bir komut bulunmuyor!"
    );
  } else {
    message.channel
      .send("`" + command + "` ismindeki komut yeniden başlatılıyor...")
      .then(m => {
        client
          .reload(command)
          .then(() => {
            m.edit(
              "`" + command + "` ismindeki komut başarıyla yeniden başlatıldı!"
            );
          })
          .catch(e => {
            m.edit(
              `Komutlar klasöründe \`${command}.js\` isminde bir dosya bulunamadı!`
            );
          });
      });
  }
