...
exports.run = async (client, message, args) => {
  ...
  if (kod) {
    ...
    return message.channel.send(
      `${message.author}, Malesef Premium Süreniz Bitmiştir. Detaylı Bilgi İçin /premium`
    );
  }
  ...
  if (message.author.id !== message.guild.owner.user.id)
    return message.channel.send(
      "Yeterli Yetkiye Sahip Değilsin! " +
        client.emojis.get("673125251961323532")
    );
  if (db.has(`premium_${message.guild.id}`) == false) {
    ...
    if (!db.fetch(`klimit31_${message.guild.id}`))
      return message.channel.send(`Rollimit sistemj zaten kapalı`);
    db.delete(`klimit31_${message.guild.id}`);
    message.channel.send(
      `:ok_hand: Koruma Rol Sistemi Sunucunuz İçin Veritabanından Silinmiştir.`
    );
  }
};
...
exports.help = {
  ...
  description: "Rol limiti sistemini kapatır.",
  usage: "rollimit"
  //  kategori: 'yetkili'
};
