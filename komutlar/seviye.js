...
const saydam = ["saydamlaştır", "saydam", "saydamlık"];
      ...
      if (reset.includes(args[1])) {
        if (!db.has(`${msg.author.id}.renk`)) {
          const embed = new Discord.RichEmbed()
            .setDescription("Renk değiştirilmemiş.")
            .setColor("RED");
    ...
    if (arm.includes(args[0])) {
      if (reset.includes(args[1])) {
        if (!db.has(`${msg.author.id}.resim`)) {
          const embed = new Discord.RichEmbed()
            .setDescription("Ayarlı bir resim yok.")
            .setColor("RED");
          msg.channel.send({ embed });
          return;
        }
      ...
      if (!args[1]) {
        const embed = new Discord.RichEmbed()
          .setDescription("Ayarlamak istediğiniz resmin linkini girin")
      ...
      if (reset.includes(args[1])) {
        if (!db.has(`roll_${msg.guild.id}`)) {
          if (!db.has(`rollss_${msg.guild.id}`)) {
            const embed = new Discord.RichEmbed()
              .setDescription("Rol ödülleri sistemi aktif değil")
              .setColor("RED");
            msg.channel.send({ embed });
            return;
          }
