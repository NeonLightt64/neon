...
exports.run = async function(client, message, args) {
  ...
  if (!x)
    return message.reply("Temizlemek istediğin mesaj sayısını yazmalısın!");
  ...
  message.channel
    ...
    .catch(error =>
      message.channel.send(
        ":rolling_eyes: `14` günden önceki mesajları silmeme Discord API izin vermiyor"
      )
    );

  message.channel.send(
    `**${u.tag}** adlı kullanıcının **${x}** adet mesajı başarıyla silindi!`
  );
