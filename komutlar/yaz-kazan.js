...
exports.run = async (client, msg, args) => {
  ...
  if (opponent.id === msg.author.id)
    return msg.reply("Kendin ile oynuyamazsın!");
  if (this.fighting.has(msg.channel.id))
    return msg.reply("Kanal başına sadece bir meydan okuma gelebilir!");
  this.fighting.add(msg.channel.id);
  try {
    await msg.channel.send(
      `${opponent}, bu meydan okumayı kabul ediyor musun? (\`evet\` veya \`hayır\` olarak cevap veriniz.)`
    );
    ...
    await msg.channel.send("Hazırlanın kelime geliyor...").then(m => {
      m.delete(3000);
    });
    const word = words[Math.floor(Math.random() * words.length)];
    await msg.channel.send(`Kelime **\`${word.toUpperCase()}\`** Hemen Yaz!`);
    await msg.channel.send(`Kelimeyi tamamen küçük harfle yazınız._`);
    const filter = res =>
    ...
    this.fighting.delete(msg.channel.id);
    if (!winner.size)
      return msg.channel.send("Kimse kazanmadı, berabere bitti!");
    return msg.channel.send(`Tebrikler ${winner.first().author} Kazandın!`);
  } catch (err) {
    this.fighting.delete(msg.channel.id);
    throw err;
  }
